...
CGFloat const contcIvAvatarSize = 40.0f;
...
- (void)initAndAddSubview
{
    ...
    ivAvatar = [[UIImageView alloc] initWithFrame:CGRectMake( horizontalSpace, verticalSpace, contcIvAvatarSize, contcIvAvatarSize)];
    [ivAvatar setContentMode:UIViewContentModeScaleAspectFill];
    [ivAvatar setClipsToBounds:YES];
    [[ivAvatar layer] setCornerRadius:contcIvAvatarSize/2.0f];
...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake( tvContent.frame.origin.x, tvContent.ly_y + tvContent.frame.size.height + verticalSpace, ctcTvEvalutionWidth, ctcTvEvalutionHeight)
...
- (void)setConsult:(LyConsult *)consult
{
    ...
    [lbName setFrame:CGRectMake(ivAvatar.frame.origin.x+ivAvatar.frame.size.width+horizontalSpace, ivAvatar.ly_y, fWidthLbName, ctcLbNameHeight)];
    ...
    [lbTime setFrame:CGRectMake(lbName.frame.origin.x, lbName.ly_y+CGRectGetHeight(lbName.frame), fWidthLbTime, ctcLbTimeHeight)];
    ...
    [tvContent setFrame:CGRectMake( contcWidth/2-ctcTvContentWidth/2, ivAvatar.ly_y+CGRectGetHeight(ivAvatar.frame)+verticalSpace, ctcTvContentWidth, fHeightTvContent)];
    ...
    if (LyConsultTableViewCellMode_detail == _mode) {
        ...
        self.tableView.frame = CGRectMake( tvContent.frame.origin.x, tvContent.ly_y+CGRectGetHeight(tvContent.frame)+verticalSpace, ctcTvEvalutionWidth, 0);
        ...
        self.lbReplyCount.frame = CGRectMake(horizontalSpace, tvContent.ly_y + CGRectGetHeight(tvContent.frame), SCREEN_WIDTH - horizontalSpace * 2, ctcLbReplyCountHeight);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( !flagForResetHeight) {
        if (_consult.arrReply.count > indexPath.row + 1) {
            ...
            self.tableView.frame = CGRectMake( tvContent.frame.origin.x, tvContent.ly_y+CGRectGetHeight(tvContent.frame), ctcTvEvalutionWidth, heightForCell);
