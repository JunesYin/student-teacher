...
- (void)setupToolbars
{
	...
	//modify by junes 2016.11.3
//    if (self.imageCount > 1) {
        indexLabel.text = [[NSString alloc] initWithFormat:@"1/%ld", (long)self.imageCount];
//    }
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
		...
		//modify by junes 2016.11.3 begin
        if ( [_delegate respondsToSelector:@selector(isAllowSaveImage:)] && ![_delegate isAllowSaveImage:self])
        {
...
- (void)layoutSubviews
{
	...
	//modify by junes 2016.11.3
    _saveButton.frame = CGRectMake(30, self.bounds.size.height - 70, 80, 25);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
	//modify by junes 2016.11.3
        _indexLabel.text = [[NSString alloc] initWithFormat:@"%d/%ld", index + 1, (long)self.imageCount];
    }
