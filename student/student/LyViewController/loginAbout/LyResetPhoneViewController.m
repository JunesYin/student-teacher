...
@interface LyResetPhoneViewController () <UITextFieldDelegate, LyHttpRequestDelegate, LyGetAuthCodeButtonDelegate, LyRemindViewDelegate>
{
    UITextField             *tfPhone;
...
- (void)initSubviews {
    ...
    tfPhone = [[UITextField alloc] initWithFrame:CGRectMake(0, lbRemind.frame.origin.y+CGRectGetHeight(lbRemind.frame), SCREEN_WIDTH, LyViewItemHeight)];
    ...
    //登录
    btnReset = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2.0f-rpBtnResetWidth/2.0f, viewAuthCode.frame.origin.y+CGRectGetHeight(viewAuthCode.frame)+20.0f, rpBtnResetWidth, rpBtnResetHeight)];
        ...
        case resetPhoneHttpMethod_reset: {
            switch ([strCode integerValue]) {
                case 0: {
                    ...
                    [[NSUserDefaults standardUserDefaults] setObject:tfPhone.text forKey:userAccount517Key];
...
- (void)remindViewDidHide:(LyRemindView *)aRemind {
    [_delegate onDoneByResetPhoneVC:self];
}
