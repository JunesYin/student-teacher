...
class LyEvaMsgDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, LySUtilDelegate, LyReplyViewDelegate {
    ...
    func reloadData_header() {
        ...
        lbScore.frame = CGRect(x: SCREEN_WIDTH - horizontalSpace - emdLbScoreWidth, y: tvContent.ly_y + fHeightTvContent + verticalSpace, width: emdLbScoreWidth, height: emdLbScoreHeight)
        lbScore.text = String(format: "%.1f", eva.score) + "分"
        
        srScore.frame = CGRect(x: lbScore.frame.origin.x - horizontalSpace - emdSrScoreWidth, y: lbScore.ly_y, width: emdSrScoreWidth, height: emdSrScoreHeight)
        srScore.setScore(eva.score / 5.0, withAnimation: false)
        
        
        ivLevel.frame = CGRect(x: srScore.frame.origin.x - horizontalSpace - emdIvLevelWidth, y: srScore.ly_y, width: emdIvLevelWidth, height: emdIvLevelHeight)
        ivLevel.image = {
            var sImageName = ""
            switch eva.level {
            case .good:
        ...
        line.frame = CGRect(x: 0, y: lbScore.ly_y + lbScore.frame.size.height + verticalSpace, width: SCREEN_WIDTH, height: verticalSpace)
