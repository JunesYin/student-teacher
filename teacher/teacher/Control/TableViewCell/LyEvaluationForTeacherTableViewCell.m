...
- (void)initSubviews
{
    ...
    //评分图像
    ivScore = [[UIImageView alloc] initWithFrame:CGRectMake(ecdtcWidth-ecdtcHorizontalSpace-ecdtcIvScoreWidth, ivAvatar.ly_y, ecdtcIvScoreWidth, ecdtcIvScoreHeight)];
    [ivScore setContentMode:UIViewContentModeScaleAspectFit];
    [self addSubview:ivScore];
    
    //评分文字
    lbScore = [[UILabel alloc] initWithFrame:CGRectMake(ivScore.frame.origin.x - 3 - ecdtcLbScoreWidth, ivScore.ly_y, ecdtcLbScoreWidth, ecdtcLbScoreHeight)];
...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake( tvContent.frame.origin.x, tvContent.ly_y + tvContent.frame.size.height + verticalSpace, ecdtcTvContentWidth, 0)
...
//        }
    ...
    [lbName setFrame:CGRectMake( ivAvatar.frame.origin.x+ivAvatar.frame.size.width+ecdtcHorizontalSpace, ivAvatar.ly_y+2, sizeLbName.width, ecdtcLbNameHeight)];
    ...
    [lbTime setFrame:CGRectMake( lbName.frame.origin.x, ivAvatar.ly_y+ivAvatar.frame.size.height-2-ecdtcLbTimeHeight, fWidthLbTime, ecdtcLbTimeHeight)];
    ...
    CGRect rectTest = CGRectMake( ecdtcHorizontalSpace, ivAvatar.ly_y+ivAvatar.frame.size.height, ecdtcTvContentWidth, fHeightTvContent);
    ...
    if (LyEvaluationForTeacherTableViewCellMode_detail == _mode) {
        ...
        [self addSubview:self.tableView];
        self.tableView.frame = CGRectMake( horizontalSpace, tvContent.ly_y + CGRectGetHeight(tvContent.frame) + verticalSpace, SCREEN_WIDTH - horizontalSpace * 2, 0);
        
        if (_eva.arrReply.count > 0) {
            [self.tableView reloadData];
        }
        
        _height = self.tableView.ly_y + CGRectGetHeight(self.tableView.frame) + verticalSpace;
        
    } else {
        [_tableView removeFromSuperview];
        [self addSubview:self.lbReplyCount];
        
        self.lbReplyCount.frame = CGRectMake(horizontalSpace, tvContent.ly_y + CGRectGetHeight(tvContent.frame), SCREEN_WIDTH - horizontalSpace * 2, ecdtcLbReplyCountHeight);
        self.lbReplyCount.text = [[NSString alloc] initWithFormat:@"回复（%ld）", _eva.replyCount];
        
        _height = self.lbReplyCount.ly_y + CGRectGetHeight(self.lbReplyCount.frame) + verticalSpace;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( !flagForResetHeight) {
        if (_eva.arrReply.count > indexPath.row + 1) {
            ...
            self.tableView.frame = CGRectMake( tvContent.frame.origin.x, tvContent.ly_y+CGRectGetHeight(tvContent.frame), ecdtcTvContentWidth, heightForCell);
            
            _height = self.tableView.ly_y + CGRectGetHeight(self.tableView.frame);
