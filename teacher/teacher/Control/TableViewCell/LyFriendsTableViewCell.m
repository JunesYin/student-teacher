...
CGFloat const maCellIvKindWidth = 60;
...
- (void)setUser:(LyUser *)user {
    ...
    [lbName setFrame:CGRectMake( ivAvatar.frame.origin.x+ivAvatar.frame.size.width+horizontalSpace, ivAvatar.ly_y, fWidthLbName, lbItemHeight)];
    [lbName setText:user.userName];
    
    
    //类别
    [ivKind setFrame:CGRectMake( lbName.frame.origin.x+lbName.frame.size.width+horizontalSpace, lbName.ly_y, maCellIvKindWidth, lbItemHeight)];
    ...
    //个人签名
    [lbSignature setFrame:CGRectMake( lbName.frame.origin.x, lbName.ly_y+lbName.frame.size.height, SCREEN_WIDTH-maCellIvKindWidth-horizontalSpace*3, fcellHeight-lbName.ly_y-lbItemHeight)];
