...
//动态输入框
CGFloat const ssTvContentHeight = 200.0f;
//动态输入框最大字符数
...
CGFloat const picVerticalMargin = 5.0f;

#define viewPicsWidth                                   FULLSCREENWIDTH
#define viewPicsHeight                                  (cvPicItemSize+picVerticalMargin*2)
...
{
    sendStatusAlertViewMode_album = 1,
};
...
- (void)ssLayoutUI
{
    ...
    UIBarButtonItem *bbiRight = [[UIBarButtonItem alloc] initWithTitle:@"发表" style:UIBarButtonItemStyleDone target:self action:@selector(ssTargetForBarBtnItemRight)];
//    UIBarButtonItem *bbiRight = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:<#(UIBarButtonSystemItem)#> target:<#(nullable id)#> action:<#(nullable SEL)#>]
    [self.navigationItem setRightBarButtonItem:bbiRight];
    
    //动态输入框
    tvContent = [[UITextView alloc] initWithFrame:CGRectMake(horizontalSpace, STATUSBARHEIGHT+NAVIGATIONBARHEIGHT+verticalSpace*2, FULLSCREENWIDTH-horizontalSpace*2, ssTvContentHeight)];
...
- (void)sendNewsForDelay
{
    ...
    {
        [indicotar_send stop];
        LyRemindView *remin = [LyRemindView remindWithMode:LyRemindViewMode_fail withTitle:remindTitle_sensitiveWord];
        [remin performSelector:@selector(show) withObject:nil afterDelay:LyRemindViewDelayTime];
    }
    else
    {
...
- (void)handleHttpFailed {
    if ([indicotar_send isAnimating]) {
        [indicotar_send stop];
        LyRemindView *remind = [LyRemindView remindWithMode:LyRemindViewMode_fail withTitle:@"发表失败"];
        [remind show];
    }
}
        ...
        case sendStatusHttpMethod_send: {
            ...
            switch ( [strCode integerValue]) {
                case 0: {
                    ...
                    [indicotar_send stop];
                    LyRemindView *remindSuccess = [LyRemindView remindWithMode:LyRemindViewMode_success withTitle:@"发表成功"];
...
{
    return UIEdgeInsetsMake( picVerticalMargin, 0, picVerticalMargin, 0);
}
