...
CGFloat const uctcellHeight = 40.0f;

//图标
CGFloat const uctcIvIconSize = 30;

//文字
#define uctcLbTitleWidth                        (SCREEN_WIDTH/2.0f)
CGFloat const uctcLbTitleHeight = uctcellHeight;
...
- (void)initAndLayoutSubView
{
    ...
    //图标
    ivIcon = [[UIImageView alloc] initWithFrame:CGRectMake( horizontalSpace, uctcellHeight/2-uctcIvIconSize/2, uctcIvIconSize, uctcIvIconSize)];
    [ivIcon setContentMode:UIViewContentModeScaleAspectFit];
    [self addSubview:ivIcon];
    
    //标题
    lbTitle = [[UILabel alloc] initWithFrame:CGRectMake( ivIcon.frame.origin.x+ivIcon.frame.size.width+horizontalSpace, 0, uctcLbTitleWidth, uctcLbTitleHeight)];
