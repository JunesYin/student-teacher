...
- (void)viewWillAppear:(BOOL)animated {
    ...
    switch (_mode) {
        case LyModifyNameControllerMode_name: {
            self.title = @"姓名";
            [tfName setText:[LyCurrentUser curUser].userName];
            break;
        }
        case LyModifyNameControllerMode_trueName: {
            self.title = @"驾校全称";
            [tfName setText:[LyCurrentUser curUser].schoolTrueName];
            break;
        }
        default:
            break;
...
- (void)modify {
    if (![tfName.text validateSensitiveWord]) {
        ...
        LyRemindView *remind = [LyRemindView remindWithMode:LyRemindViewMode_fail withTitle:LyRemindTitle_sensitiveWord];
        [remind show];
        return;
    }
    ...
    switch (_mode) {
        case LyModifyNameControllerMode_name: {
            [dic setObject:tfName.text forKey:nickNameKey];
            break;
        }case LyModifyNameControllerMode_trueName: {
            [dic setObject:tfName.text forKey:fullNameKey];
            break;
        }
        default:
            break;
           ...
           if (error) {
               ...
               switch (strCode.integerValue) {
                   case 0: {
                       ...
                       if (LyModifyNameControllerMode_name == _mode) {
                           ...
                           NSString *newFullName = [[NSString alloc] initWithFormat:@"%@", [dicResult objectForKey:fullNameKey]];
                           [[LyCurrentUser curUser] setSchoolTrueName:newFullName];
