...
- (void)setDelegate:(id<StarRatingViewDelegate>)delegate
{
    if ( !delegate)
    {
        return;
    ...
    [self changeStarForegroundViewWithPoint:CGPointMake( 0, 5)];
}
...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] pathForResource:@"resource" ofType:@"bundle"] stringByAppendingFormat:@"/images/%@", imageName]]];
//        UIImageView *imageView = [[UIImageView alloc] initWithImage:[LyUtil imageForImageName:imageName needCache:NO]];
...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    NSString * str = [[NSString alloc] initWithFormat:@"%0.2f",p.x / self.frame.size.width];
