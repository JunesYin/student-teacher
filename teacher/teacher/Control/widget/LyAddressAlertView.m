...
- (void)initAndLayoutSubviews {
    ...
    picker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, toolBar.ly_y+CGRectGetHeight(toolBar.frame), CGRectGetWidth(viewUseful.frame), aavPickerHeight)];
    [picker setDelegate:self];
    [picker setDataSource:self];
//    [picker setShowsSelectionIndicator:YES];
    
    
    tvContent = [[UITextView alloc] initWithFrame:CGRectMake(verticalSpace, toolBar.ly_y+CGRectGetHeight(toolBar.frame)+CGRectGetHeight(picker.frame)+verticalSpace, CGRectGetWidth(viewUseful.frame)-verticalSpace*2, CGRectGetHeight(viewUseful.frame)-CGRectGetHeight(toolBar.frame)-CGRectGetHeight(picker.frame)-verticalSpace*2)];
