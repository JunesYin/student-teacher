...
- (void)setStrUrl:(NSString *)strUrl {
    ...
    [ivPic sd_setImageWithURL:[NSURL URLWithString:_strUrl]
...
CGFloat const ntcBtnFuncHeight = 25.0f;
...
- (void)setNews:(LyNews *)news mode:(LyNewsTableViewCellMode)mode {
    ...
    [lbName setFrame:CGRectMake( ivAvatar.frame.origin.x+ivAvatar.frame.size.width+horizontalSpace, ivAvatar.ly_y, fWidthLbName, ntcNameHeight)];
    ...
    CGRect rectStcTime = CGRectMake( lbName.frame.origin.x, lbName.ly_y+lbName.frame.size.height, fWidthLbTime, ntcTimeHeight);
    ...
    if (![LyUtil validateString:_news.newsContent] ){
        [tvContent setText:@""];
        [tvContent setFrame:CGRectMake( horizontalSpace, ivAvatar.ly_y+CGRectGetHeight(ivAvatar.frame), ntcContentWidth, verticalSpace)];
    } else {
        ...
        [tvContent setFrame:CGRectMake( horizontalSpace, ivAvatar.ly_y+CGRectGetHeight(ivAvatar.frame), ntcContentWidth, fHeightTvContent)];
    }
    ...
    [btnTransimit setFrame:CGRectMake( SCREEN_WIDTH-horizontalSpace-fWidthBtnTransmit, viewPic.ly_y+viewPic.frame.size.height+ntcVerticalSpace, fWidthBtnTransmit, ntcBtnFuncHeight)];
    ...
    fWidthBtnEvaluation = ( fWidthBtnEvaluation < 45) ? 45 : fWidthBtnEvaluation;
    [btnEvalution setFrame:CGRectMake( btnTransimit.frame.origin.x-horizontalSpace*2-fWidthBtnEvaluation, btnTransimit.ly_y, fWidthBtnEvaluation, ntcBtnFuncHeight)];
    ...
    [btnPraise setFrame:CGRectMake( btnEvalution.frame.origin.x-horizontalSpace*2-fWidthBtnPraise, btnEvalution.ly_y, fWidthBtnPraise, ntcBtnFuncHeight)];
    [btnPraise setNumber:_news.newsPraiseCount];
    
    [btnPraise praise:_news.isPraised];
    
    [_news setCellHeight:btnPraise.ly_y + btnPraise.frame.size.height + ntcVerticalSpace];
    
    switch ( _mode) {
        case LyNewsTableViewCellMode_community: {
            
            _cellHeight = btnPraise.ly_y + btnPraise.frame.size.height + ntcVerticalSpace;
        ...
        case LyNewsTableViewCellMode_detail: {
            ...
            _cellHeight = viewPic.ly_y + viewPic.frame.size.height + ntcVerticalSpace;
...
- (void)stcSetPic {
    ...
    if ( 0 == iPicNum) {
        [viewPic setFrame:CGRectMake( horizontalSpace, tvContent.ly_y+CGRectGetHeight(tvContent.frame), viewPicWidth, 0)];
        [ivPic setHidden:YES];
        [self removeGestureFromIvPic];
        [cvPics setHidden:YES];
    } else if ( 1 == iPicNum) {
        ...
        [viewPic setFrame:CGRectMake( horizontalSpace, tvContent.ly_y+CGRectGetHeight(tvContent.frame)+verticalSpace, viewPicWidth, CGRectGetHeight(ivPic.frame))];
        [ivPic setHidden:NO];
        [self addGestureToIvPic];
        [cvPics setHidden:YES];
        ...
        [viewPic setFrame:CGRectMake( horizontalSpace, tvContent.ly_y+CGRectGetHeight(tvContent.frame)+verticalSpace, viewPicWidth, cvPics.ly_y+CGRectGetHeight(cvPics.frame))];
...
- (void)removeGestureFromIvPic {
    for (UITapGestureRecognizer *gesture in ivPic.gestureRecognizers) {
        [ivPic removeGestureRecognizer:gesture];
    }
}
