...
NSString *const LyPinyinGroupResultKey = @"LyPinyinGroupResultKey";

NSString *const LyPinyinGroupNameKey = @"LyPinyinGroupNameKey";

//static char LyPinyinGroupSectionNumberKey;

@implementation LyPinyinGroup
...
//        objc_setAssociatedObject(model, &LyPinyinGroupSectionNumberKey, @(sectionNumber), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
//    }
...
//        NSInteger sectionNumber = [objc_getAssociatedObject(model, &LyPinyinGroupSectionNumberKey) integerValue];
//        [newSectionArray[sectionNumber] addObject:model];
        NSString *strXXX = [[model valueForKey:key] description];
        unichar firstChar = [LyPinyinGroup firstCharactor:strXXX];
        int sectionNumer = firstChar - 'a';
        if ( sectionNumer < 0)
        {
            sectionNumer = firstChar - 'A';
        }
        if ( sectionNumer < 0)
        {
            sectionNumer = 26;
        }
        [newSectionArray[sectionNumer] addObject:model];
        NSString *firstCharString = [[[NSString alloc] initWithFormat:@"%C",firstChar] uppercaseString];
...
//    NSArray *charSortArray = [charArray sortedArrayUsingSelector:@selector(compare:)];
    NSArray *charSortArray = [charArray sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        ...
        if ([obj1 isEqualToString:@"#"]) {
            ...
            result = [obj1 compare:obj2 options:NSCaseInsensitiveSearch];
        }
    ...
    return @{LyPinyinGroupNameKey:charSortArray,
             LyPinyinGroupResultKey:resultArray};
}
