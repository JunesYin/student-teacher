...
class LyMsgCenterTableViewController: UITableViewController, LySUtilDelegate {
    ...
    let indicator: LyIndicator? = LyIndicator(title: nil)
        ...
        LyHttpRequest.start(msgCenter_url,
                            ...
                            timeOut: 0) { [unowned self] (resStr, resData, error) in
                                ...
                                case 0:
                                    ...
                                    let iEvaRepCount = dicResult?[conRepMsgCountKey]?.intValue
                                    let iConRepCount = dicResult?[evaRepMsgCountKey]?.intValue
                                    
                                    if nil != iEvaCount && nil != iEvaRepCount {
                                        self.iEvaCount = iEvaCount! + iEvaRepCount!
                                    }
                                    if nil != iConCount && nil != iConRepCount {
                                        self.iConCount = iConCount! + iConRepCount!
                                    }
...
extension LyMsgCenterTableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            sTitle = LySLocalize("评价消息")
            sDetail = "有\(iEvaCount)人评价了你"
            iCount = iEvaCount
        case 1:
            iIcon = LyUtil.image(forImageName: "msgCenter_con", needCache: false)
            sTitle = LySLocalize("咨询消息")
            sDetail = "有\(iConCount)人向你提问了"
            iCount = iConCount
        default:
            iIcon = nil
            sTitle = ""
