...
@interface LyModifyPhoneViewController () <UITextFieldDelegate, LyGetAuthCodeButtonDelegate, LyHttpRequestDelegate>
{
...
- (void)initAndLayoutSubviews {
    ...
    tfPhone = [[UITextField alloc] initWithFrame:CGRectMake(0, lbRemindUpper.ly_y+CGRectGetHeight(lbRemindUpper.frame)+verticalSpace*2, SCREEN_WIDTH, LyViewItemHeight)];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (modifyPhoneTextFieldTag_phone == textField.tag) {
        if (tfPhone.text.length + string.length > LyAccountLength) {
            return NO;
        }
    } else if (modifyPhoneTextFieldTag_authCode == textField.tag) {
